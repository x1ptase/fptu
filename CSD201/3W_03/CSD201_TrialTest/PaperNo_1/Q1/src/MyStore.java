...
class FruitList {
    ...
    void addLast(String type, int amount, int price) {
        ...
        //--------------------------------------------------------
        Fruit fruit = new Fruit(type, amount, price);
        Node p = new Node(fruit, null);
        if(isEmpty()){
            head = tail = p;
        } else{
            tail.next = p;
...
class MyStore {
    ...
    void purchaseAction(Fruit request){
        if(request == null){
            return;
        }
        Node p = FList.head;
        ...
        if(fruitNode != null){
            if(fruitNode.info.getAmount()>= request.getAmount()){
                int newAmount = fruitNode.info.getAmount()- request.getAmount();
                fruitNode.info.setAmount(newAmount);
            }
        }
    }
    ...
    void f2() throws Exception {
        ...
        //You should write here appropriate statements to complete this function.
        //--------------------------------------------------------
        Fruit purchaseReq = RQueue.deQueue();
    ...
    int calculateTotal(Fruit request) {
        if(request == null) {
            return 0;
        }
        Node p = FList.head;
        ...
        if(fruitNode != null){
            if(fruitNode.info.getAmount() >= request.getAmount()){
                int newAmount = fruitNode.info.getAmount() - request.getAmount();
                fruitNode.info.setAmount(newAmount);
                return request.getAmount() * fruitNode.info.getPrice();
            }
        }
        return 0; 
    }
